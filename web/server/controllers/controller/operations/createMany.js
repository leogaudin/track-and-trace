...
async function processInstances(instances, Model, apiKeyNeeded) {
  ...
  if (apiKeyNeeded) {
    try {
      await requireApiKey(req, res, async () => apiKeyChecked = true);
    } catch (error) {
...
const createMany = (Model, apiKeyNeeded = true) => async (req, res) => {
  try {
    ...
    processInstances(instances, Model, apiKeyNeeded);
  } catch (error) {
